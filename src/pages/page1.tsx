...
export default function Page1() {
  ...
  return (
    <div className="overflow-hidden h-full w-screen">
      {/* Section Heading */}
      <div className="w-full h-full flex flex-col gap-10 p-32 font-NeueMachina-Regular z-50">
        <h2 className="text-3xl md:text-5xl font-NeueMachina-Bold">
          Why <span className="text-yellow-400">MoonEX </span>?
        </h2>

        {/* Comparison Table */}
        <div className="w-full bg-white bg-opacity-5 backdrop-blur-sm rounded-3xl shadow-lg p-10 z-50">
          <table className="w-full text-left border-collapse z-50">
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr key={index}>
                  <td className="py-8 pl-4 text-gray-300">
                    {index + 1}. {row}
                  </td>
